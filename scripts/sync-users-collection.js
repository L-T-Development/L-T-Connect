    ...
    try {
        ...
        try {
            ...
            if (error.code === 404) {
                ...
                collection = await databases.createCollection(DATABASE_ID, COLLECTION_ID, 'App Users', 
        ...
        for (const attr of attributes) {
            ...
            try {
                if (attr.type === 'string') {
                    await databases.createStringAttribute(DATABASE_ID, COLLECTION_ID, attr.key, attr.size, attr.required, attr.default, attr.array);
                }
                else if (attr.type === 'boolean') {
                    await databases.createBooleanAttribute(DATABASE_ID, COLLECTION_ID, attr.key, attr.required, attr.default, attr.array);
                }
        ...
        const existingIndexKeys = existingIndexes.indexes.map((idx) => idx.key);
        for (const index of indexes) {
            ...
            try {
                await databases.createIndex(DATABASE_ID, COLLECTION_ID, index.key, index.type, index.attributes);
